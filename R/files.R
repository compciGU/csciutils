    ...
    fn <- switch(ext,
      ...
      dta = {
        if (haven_avail) {
          function(path, ...) {
            dots <- list(...)
            # If user provided encoding, respect it
            if (!is.null(dots$encoding)) return(haven::read_dta(path, ...))
            # Try default; on error try Latin-1, then fallback to readstata13 if available
            tryCatch(
              haven::read_dta(path, ...),
              error = function(e) {
                tryCatch(
