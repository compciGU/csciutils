...
get_cwt <- function(cwt = NULL,
  ...
  if (is.null(cwt)) {
    ...
    if (verbose)
      message("No existing cwt supplied. Building CWT from scratch.")

    cwt <- build_cwt(my_survey_list, variable_map, verbose = verbose)        # find helper function in cwt_build

    if (save_file)
      save_cwt(                       # find helper function in cwt_write
        cwt = cwt,
        output_dir = dir,
  ...
  validate_append_item(cwt, my_survey_list, variable_map, dir, out_dir)     # find helper function in cwt_validate_input

  has_waves <- length(unique(cwt$study_wave)) > 1

  if (align_surveys_with_cwt && has_waves) {
    my_survey_list <- align_survey_to_cwt(my_survey_list, cwt, verbose = verbose)        # find helper function in cwt_align_vars
  }

  vars_missing_df <- find_missing_vars(cwt, variable_map, has_waves)                    # find helper function in cwt_align_vars

  if (nrow(vars_missing_df) == 0) {
    if (verbose)
      message("No missing variables to add. Returning original cwt unchanged.")
  ...
  if (verbose)
    message(paste("Found", nrow(vars_missing_df),
        "missing variable(s). Building CWT rows."))

  cwt_new  <- build_cwt(my_survey_list, vars_missing_df, verbose = verbose)                 # find helper function in cwt_build
  ...
  if (verbose)
    message(paste("Appended", nrow(cwt_new), "new row(s) to CWT."))
